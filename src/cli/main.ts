...
options:
  --config <path>     litmus.config.yaml to use (default: ./litmus.config.yaml)
  -h, --help          this text`
    ...
    switch (command) {
      case 'list': {
        const { values, positionals } = parseArgs({
          ...
          options: { config: { type: 'string', default: 'litmus.config.yaml' } },
        })
        return list(values.config, positionals, io)
      }
      default:
