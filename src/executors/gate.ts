...
const NETWORK = new Set(['WebFetch', 'WebSearch'])
...
export function decide(tool: string, input: Record<string, unknown>, p: GatePolicy): Decision {
  ...
  for (const field of reads ?? writes!) {
    ...
    const real = realpathOf(isAbsolute(target) ? target : resolve(p.workdir, target))
...
// does not exist yet (a file about to be written) is appended as-is.
